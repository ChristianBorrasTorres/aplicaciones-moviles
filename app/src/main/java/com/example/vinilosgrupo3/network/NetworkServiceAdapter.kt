...
class NetworkServiceAdapter constructor(context: Context) {
    ...
    fun getMusicianDetail(musicianId:Int, onComplete:(resp:Musician)->Unit, onError: (error:VolleyError)->Unit){
        requestQueue.add(getRequest("musicians/$musicianId",
            Response.Listener<String> { response ->
                val resp = JSONObject(response)
                var item = Musician(musicianId = resp.getInt("id"),name = resp.getString("name"), image = resp.getString("image"), description = resp.getString("description"), birthDate = resp.getString("birthDate"))
                Log.d("Args", item.toString())
                onComplete(item)
            },
            Response.ErrorListener {
